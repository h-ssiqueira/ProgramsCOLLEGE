...
void mostrar(struct reg funcionarios[],int n){
    for(int i = 0;i < n;i++){
        printf("\n\n\t%s.\n\t%d anos.\n\tIDD: %s.\n\tSalario: R$%.2f.",funcionarios[i].nome,funcionarios[i].idade,funcionarios[i].idd,funcionarios[i].salario);
        if(funcionarios[i].idade > 25)
            printf("\n\t%s parabens vc tem mais de 25 anos",);
        }
}
